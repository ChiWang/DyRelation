...
void Bgo_RelStat(TString fname,int entries_cut = 100)
{
   ...
   DAMPE::Bgo::LoadRelation((std::string)fname,rel_par,steering);
   ...
   for(short s=0;s<2;++s){
     for(short dy=0;dy<2;++dy){
       c[s*2+dy+1]->Divide(2,2);
       c[s*2+dy+1]->cd(1);
       h_p0p1[s][dy]->Draw();
       c[s*2+dy+1]->cd(2);
       h_pileupEvts[s][dy]->Draw("colz");
       c[s*2+dy+1]->cd(3);
       h_chi[s][dy]->Draw("colz");
       c[s*2+dy+1]->cd(4);
       h_pileupEvtsRatio[s][dy]->Draw("colz");
     }
   }
...
void Psd_RelStat(TString fname,int entries_cut = 100)
{
   DmpParameterSteering  steering;
   DmpParameterHolder  rel_par;
   DAMPE::Psd::LoadRelation((std::string)fname,rel_par,steering);

   TH2F*    h_p0p1[2];
   TH2F*    h_chi[2];
   TH2F*    h_pileupEvts[2];
   TH2F*    h_pileupEvtsRatio[2];
   TCanvas *c[2];
   ...
   for(short s=0;s<2;++s){
       ...
       h_pileupEvts[s] = new TH2F(xx+" pileup events","pileup events;bar ID;layer ID",41,0,41,2,0,2);
       h_pileupEvtsRatio[s] = new TH2F(xx+" pileup events ratio","pileup ratio;bar ID;layer ID",41,0,41,2,0,2);
       c[s+1] =new TCanvas(xx,xx);
   }
   //gStyle->SetOptStat(000000);
   gStyle->SetOptStat("emru");

   int entries = 0, errorEvts = 0;
   short lid =0,bid = 0,sid = 0;
   for(DmpParameterHolder::iterator it = rel_par.begin(); it != rel_par.end();++it){
     entries = it->second.at(7);
     if(entries < entries_cut) continue;
     lid  = it->second.at(0);
     bid  = it->second.at(1);
     sid  = it->second.at(2);
     h_p0p1[sid]->Fill(it->second.at(5),it->second.at(4));
     h_chi[sid]->Fill(bid,lid,it->second.at(6));
     errorEvts = it->second.at(8);
     h_pileupEvts[sid]->Fill(bid,lid,errorEvts);
     h_pileupEvtsRatio[sid]->Fill(bid,lid,(double)errorEvts /  (double)entries * 100);
   }
   ...
   for(short s=0;s<2;++s){
       ...
       h_p0p1[s]->Draw();
       c[s+1]->cd(2);
       h_pileupEvts[s]->Draw("colz");
       c[s+1]->cd(3);
       h_chi[s]->Draw("colz");
       c[s+1]->cd(4);
       h_pileupEvtsRatio[s]->Draw("colz");
   }
